...
export async function fetchGitHubStats(token: string): Promise<RawGitHubStats> {
  const now = new Date();
  ...
  for (const repo of repos) {
    ...
    if (branch) {
      ...
      for (const edge of branch.target.history.edges) {
        const d = new Date(edge.node.committedDate);
        commitHours.push(d.getHours());
      }
    }
  }
...
export async function fetchGitHubStatsForUser(
  token: string,
  username: string
...
`;
  ...
  for (const repo of repos) {
    ...
    const created = new Date(repo.createdAt);
    const age = (now2.getTime() - created.getTime()) / (1000 * 60 * 60 * 24);
    repoAges.push(age);
    repoPushedAts.push(repo.pushedAt);
    const branch = repo.defaultBranchRef;
    if (branch) {
  ...
  const recentCommits = contributionDays
    .filter((d: ContributionDay) => {
      const day = new Date(d.date);
      const daysAgo = (now2.getTime() - day.getTime()) / (1000 * 60 * 60 * 24);
      return daysAgo <= 30 && daysAgo >= 0;
    })
  ...
  for (const day of sortedDays) {
    const daysAgo = (now2.getTime() - new Date(day.date).getTime()) / (1000 * 60 * 60 * 24);
    if (daysAgo > 365) break;
    if (day.contributionCount > 0) {
      streak++;
      longestStreak = Math.max(longestStreak, streak);
